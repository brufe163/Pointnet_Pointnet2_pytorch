...
def vis_result_with_metrics_matplotlib(coord, pred_classes, true_classes, area_name):
    colors = np.zeros((len(pred_classes), 3))
    ...
    # Graficar los puntos con sus colores
    ax.scatter(coord[:, 0], coord[:, 1], coord[:, 2], c=colors, s=1)

    # Configurar etiquetas y título
    ax.set_title(f"Visualización de métricas - Área {area_name}")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")

    # Ajustar las escalas para que sean iguales
...
def model_eval_single_frame(model, root='', feat=['coord', 'intensity'], num_classes=2, labels_available=True, test_areas=[1, 2, 3]):
    """
    ...
    with torch.no_grad():
        ...
        #print(f"Frame 0 - Nombre de la sala: {room_names[r_idx]}")
        ...
        # Visualizar los resultados con enfoque en puntos de polvo usando Matplotlib
        vis_result_with_metrics_matplotlib(data[:, :3], predicted_classes, labels if labels_available else None, area_num)
