...
def vis_result(coord, classes):
    ...
    # Centro extrado de una visualizacion random
    ...
    for i in range(len(classes)):
        if classes[i] == 0:
            ...
            colors[i] = colormap(distances_normalized[i])[:3]  
...
def model_eval(model, root = '', feat = ['coord','intensity'], data_set = 'test', num_classes=2):    # Para PointNet++
    ...
    DATASET = AMTCDataset(split=data_set, data_root=root, num_point=5000, voxel_size = 0.1, test_area=9, feats = feat, num_classes = num_classes)
