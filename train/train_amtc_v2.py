import argparse
import os
from data_utils.AMTCDataset import *  # Adjusted import statement
import torch
import datetime
import logging
...
"""
v2: usa porcentajes para crear conjuntos de test, train y val. Además, añade más métricas.
"""
...
def save_info(args, outdir):    
    ...
    with open(info_file, 'w') as f:
        ...
        f.write(f"split_ratios: {args.split_ratios}\n")
...
def parse_args():
    ...
    parser.add_argument('--split_ratios', nargs=3, type=float, default=[0.6, 0.2, 0.2],
                        help='Porcentajes de división para train, val y test (default: 0.6 0.2 0.2)')
...
def collate_fn(batch):
    ...
    max_points = max(points.shape[0] for points, _, _, _ in batch)
    points_padded = []  # Lista para almacenar las pointclouds y las etiquetas
    labels_padded = []

    for points, labels, _, _ in batch:
...
def main(args):
    ...
    log_string("The number of validation data is: %d" % len(dataset.val))
    ...
    classifier = MODEL.get_model(NUM_CLASSES, NUM_FEATS + 3).cuda()  # +3 por las coordenadas normalizadas
    ...
    if args.freeze_layers:
        ...
        layers_to_train = ['conv2', 'conv3', 'conv4', 'conv5', 'bn2', 'bn3', 'bn4', 'bn5']  # Ajusta según tu modelo
        for name, param in classifier.named_parameters():
    ...
    try:
        for epoch in range(start_epoch, args.epoch):
            ...
            with torch.no_grad():
                ...
                if mIoU >= best_iou:
                    best_iou = mIoU
                    best_epoch_iou = epoch + 1  # Sumamos 1
                ...
                if val_acc_avg >= best_acc:
                    ...
                    best_epoch_acc = epoch + 1  # Sumamos 1
                    logger.info('Save best model...')
