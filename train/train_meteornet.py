import argparse
import os
from data_utils.AMTCDataLoaderT import *
import torch
import datetime
import logging
...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='meteornet_sem_seg_amtc', help='Nombre del modelo [default: meteornet_sem_seg_amtc]')
    parser.add_argument('--data_dir', type=str, default='/ruta/a/tus/datos', help='Ruta a los datos [default: None]')
    parser.add_argument('--batch_size', type=int, default=8, help='Tamaño del batch durante el entrenamiento [default: 8]')
    ...
    parser.add_argument('--npoint', type=int, default=2048, help='Número de puntos [default: 2048]')
    ...
    parser.add_argument('--dropout', type=float, default=0.5, help='Probabilidad de dropout [default: 0.5]')
    ...
    parser.add_argument('--sequence_length', type=int, default=4, help='Longitud de las secuencias temporales [default: 4]')
...
def plot_and_save_metrics(train_loss_history, val_loss_history, iou_history, train_acc_history, val_acc_history, val_acc_avg_history, results_dir):
    sns.set(style="whitegrid", font_scale=1.2)
    ...
    plt.title('Precisión vs Épocas', fontsize=16)
    plt.xlabel('Épocas', fontsize=14)
    ...
    plt.close()
    print(f"Gráfico de precisión guardado en {acc_image_path}")
...
def collate_fn(batch):
    # batch es una lista de tuplas (sequence_data, sequence_labels)
    # Cada sequence_data es un array de forma [T, N_i, C]
    # Necesitamos padear las secuencias para que todas tengan el mismo número de puntos por frame

    batch_size = len(batch)
    T = len(batch[0][0])  # Longitud de secuencia

    # Encontrar el número máximo de puntos en cada frame a lo largo del batch
    max_points_per_frame = []
    for t in range(T):
        max_points = max(sequence_data[t].shape[0] for sequence_data, _ in batch)
        max_points_per_frame.append(max_points)

    C = batch[0][0][0].shape[1]  # Número de canales

    # Inicializar arrays para almacenar los puntos y etiquetas paddeados
    points_padded = np.zeros((batch_size, T, max(max_points_per_frame), C))
    labels_padded = np.zeros((batch_size, T, max(max_points_per_frame)), dtype=np.int64)

    for i, (sequence_data, sequence_labels) in enumerate(batch):
        for t in range(T):
            N = sequence_data[t].shape[0]
            points_padded[i, t, :N, :] = sequence_data[t]
            labels_padded[i, t, :N] = sequence_labels[t]
...
def evaluate_and_generate_metrics(model_name, classifier, data_loader, dataset_name, results_dir):
    # (Modificada para manejar secuencias)
    classifier.eval()
    all_pred_labels = []
    all_true_labels = []
    ...
    with torch.no_grad():
        ...
        for i, (points, target) in tqdm(enumerate(data_loader), total=len(data_loader), smoothing=0.9):
            ...
            labelweights += tmp
            all_pred_labels.append(pred_choice)
            all_true_labels.append(batch_label)
...
def main(args):
    ...
    SEQUENCE_LENGTH = args.sequence_length  # Nuevo parámetro
    ...
    print("Train Set:")
    TRAIN_DATASET = AMTCDatasetT(
        split='train', 
        data_root=ROOT, 
        num_point=args.npoint, 
        voxel_size=VOXEL_SIZE, 
        val_test_area=val_test_set,
        feats=FEATS, 
        num_classes=NUM_CLASSES, 
        labels_available=True,
        transform=TRANSFORMS,
        sequence_length=SEQUENCE_LENGTH  # Añadimos el parámetro de longitud de secuencia
    )
    
    print("Validation Set:")
    VAL_DATASET = AMTCDatasetT(
        split='val', 
        data_root=ROOT, 
        num_point=args.npoint, 
        voxel_size=VOXEL_SIZE, 
    ...
    log_string("The number of training sequences is: %d" % len(TRAIN_DATASET))
    log_string("The number of validation sequences is: %d" % len(VAL_DATASET))
    log_string("The number of test sequences is: %d" % len(TEST_DATASET))

    '''MODEL LOADING'''
    MODEL = importlib.import_module(args.model)  # Asegúrate de que meteornet.py está en la carpeta models
    ...
    classifier = MODEL.MeteorNet(NUM_CLASSES, NUM_FEATS + 3, num_frames=SEQUENCE_LENGTH, dropout=DROPOUT).cuda()  # +3 por las coordenadas normalizadas
    ...
    try:
        for epoch in range(start_epoch, args.epoch):
            ...
            with torch.no_grad():
                ...
                for i, (points, target) in tqdm(enumerate(valDataLoader), total=len(valDataLoader), smoothing=0.9):
                    ...
                    tmp, _ = np.histogram(batch_label, bins=range(NUM_CLASSES + 1))
                ...
                mIoU = np.mean(np.array(total_correct_class) / (np.array(total_iou_deno_class) + 1e-6))
                avg_class_acc = np.mean(np.array(total_correct_class) / (np.array(total_seen_class) + 1e-6))
                overall_acc = total_correct / float(total_seen)

                log_string('Validation mean loss: %f' % (loss_sum / num_batches))
                val_loss_history.append(loss_sum / num_batches)
                log_string('Validation point accuracy: %f' % overall_acc)
                val_acc_history.append(overall_acc)
                log_string('Validation point avg class acc: %f' % avg_class_acc)
                val_acc_avg_history.append(avg_class_acc)
                log_string('Validation point avg class IoU: %f' % mIoU)
                ...
                for l in range(NUM_CLASSES):
                    iou = total_correct_class[l] / (total_iou_deno_class[l] + 1e-6)
                    acc = total_correct_class[l] / (total_seen_class[l] + 1e-6)
                    iou_per_class_str += 'Class %s (%d) IoU: %.4f, Accuracy: %.4f\n' % (
                        seg_label_to_cat[l], l, iou, acc)
                ...
                if mIoU >= best_iou:
                    ...
                    logger.info('Save best model based on mIoU...')
                    savepath = str(checkpoints_dir) + '/best_model_iou.pth'
                    log_string('Saving at %s' % savepath)
                    state = {
                        'epoch': epoch,
                        'class_avg_iou': mIoU,
                        'class_avg_acc': avg_class_acc,
                        'model_state_dict': classifier.state_dict(),
                        'optimizer_state_dict': optimizer.state_dict(),
                    }
                    torch.save(state, savepath)
                    log_string('Best model saved based on mIoU')

                # Guardar el mejor modelo basado en precisión promedio de clase
                if avg_class_acc >= best_acc:
                    best_acc = avg_class_acc
                    best_epoch_acc = epoch + 1
                    logger.info('Save best model based on avg class accuracy...')
                    ...
                    torch.save(state, savepath)
                    log_string('Best model saved based on avg class accuracy')

                log_string('Current Best mIoU: %f at epoch %d' % (best_iou, best_epoch_iou))
                log_string('Current Best Avg Class Acc: %f at epoch %d' % (best_acc, best_epoch_acc))
                global_epoch += 1
        ...
        def evaluate_on_all_datasets():
            ...
            for model_name, model_path in [('best_iou', str(checkpoints_dir) + '/best_model_iou.pth'), ('best_acc', str(checkpoints_dir) + '/best_model_acc.pth')]:
